...
export default function FeaturedPostManager() {
  ...
  const { isDark } = useTheme(); // optional, can be used for conditional logic
  ...
  const fetchPosts = async () => {
    try {
      ...
      const { data: featured } = await api.get('/posts', {
        params: {
          status: 'published',
          is_featured: true,
      ...
      const { data: available } = await api.get('/posts', {
        params: {
          status: 'published',
          is_featured: false,
          per_page: 50,
  ...
  const handleDragEnd = async () => {
    ...
    try {
      ...
      await api.post('/posts/featured/reorder', { posts: updates });
    } catch (error) {
  ...
  const addToFeatured = async (post: Post) => {
    try {
      ...
      setFeaturedPosts([...featuredPosts, { ...post, is_featured: true, featured_order: featuredPosts.length + 1 }]);
  ...
  const removeFromFeatured = async (post: Post) => {
    try {
      ...
      setAvailablePosts([{ ...post, is_featured: false, featured_order: 0 }, ...availablePosts]);
    } catch (error) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Featured Posts Manager</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-1">Drag and drop to reorder featured posts</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                {featuredPosts.map((post, index) => (
                  <div
                    ...
                    className={`p-4 border-2 ... ${draggingIndex === index ? 'opacity-50' : ''}`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <button
                        ...
                        className="shrink-0 p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded"
        ...
        <div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div className="mb-4">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Available Posts</h2>
              <input
                type="text"
                value={searchQuery}
            ...
            <div className="space-y-2 max-h-600px overflow-y-auto">
              {filteredAvailablePosts.length === 0 ? (
                ...
                filteredAvailablePosts.map((post) => (
                  <div
                    key={post.id}
                    className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-gray-300 dark:hover:border-gray-600 transition bg-white dark:bg-gray-800"
                  >
                    <div className="flex items-start gap-3">
                      {post.featured_image && (
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-gray-900 dark:text-white text-sm truncate">{post.title}</h3>
                        {post.category && (
                          <span
                            className="inline-block mt-1 px-2 py-0.5 text-xs rounded"
                            style={{
                      ...
                      <button
                        ...
                        className="shrink-0 px-3 py-1 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white text-sm rounded"
                      >
                        Feature
                      </button>
                    </div>
