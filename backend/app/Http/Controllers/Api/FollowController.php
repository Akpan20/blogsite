...
class FollowController extends Controller
{
    ...
    public function suggestions(): JsonResponse
    {
        ...
        // Main suggestions: users followed by people I follow (friends-of-friends)
        $suggestions = User::where('followers', 'elemMatch', ['follower_id' => ['$in' => $followingIds]])
            ->whereNotIn('_id', array_merge($followingIds, [(string) $currentUser->_id]))
            ->select(['_id', 'name', 'username', 'avatar', 'bio', 'reputation_points'])
            ->orderBy('reputation_points', 'desc')   // safer than counting
            ->limit(10)
            ->get();
